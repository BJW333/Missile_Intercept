"""
Unified fighter training script (BC + PPO) for missile_intercept sim.

Usage:
    python train_fighter.py

What it does:
    1. Defines a light-weight FighterEnv (fighter vs PN-chasing missile).
    2. Optionally collects behavioral-cloning (BC) data from a heuristic "expert"
       and pretrains the fighter policy.
    3. Runs PPO to learn an evasive policy.
    4. Saves the final fighter policy weights to `fighter_policy.pt`, which
       matches FighterPolicyNet in missle_intercept.py and can be loaded by
       FighterRLPolicy(model_path="fighter_policy.pt").
"""
...
# ---------------------------------------------------------------------------
# CHANGE THIS IF YOUR SIM FILE NAME IS DIFFERENT
# ---------------------------------------------------------------------------
import missile_intercept as sim

TRAINING_MISSILE_INITIAL_SPEED = 750.0  # m/s, approximate post-boost speed

# ===========================================================================
# Environment: fighter vs simplified PN missile
# ===========================================================================
...
# ===========================================================================
# Networks: policy = FighterPolicyNet (matches sim), value = separate MLP
# ===========================================================================
...
class ValueNet(nn.Module):
    """
    Simple value function V(s) approximator.
    """
...
class PPOConfig:
    def __init__(self):
        ...
        self.total_steps = 4_000_000  # was at 3M
...
def compute_gae(rewards, values, dones, gamma, lam):
    ...
    rewards, values, dones: 1D torch tensors (T)
    values has length T+1 (bootstrapped last value).
...
def collect_bc_dataset(env, n_episodes=200):
    """
    Roll out heuristic expert for a bunch of episodes and collect (obs, action).
    """
    obs_list = []
    act_list = []
    ...
    for ep in range(n_episodes):
        ...
        while not done:
            action = env.expert_action(t_global)

            obs_list.append(obs.copy())
            act_list.append(action.copy())

            obs, reward, done, info = env.step(action, t_global)
            t_global += env.dt

    obs_array = np.array(obs_list, dtype=np.float32)
    act_array = np.array(act_list, dtype=np.float32)
    print(f"[BC] Collected {len(obs_array)} samples.")
    return obs_array, act_array


def train_bc(policy_net, env, device="mps",
             n_episodes=200, bc_epochs=20, batch_size=4096):
    """
    Simple supervised MSE training:
       target = expert_action(obs)
    """
    policy_net.to(device)
    policy_net.train()

    obs_array, act_array = collect_bc_dataset(env, n_episodes=n_episodes)
    ...
    act_tensor = torch.from_numpy(act_array).to(device)
    ...
    for epoch in range(bc_epochs):
        ...
        print(f"[BC] epoch {epoch+1}/{bc_epochs}, loss={epoch_loss / max(count,1):.4f}")
...
def train_ppo(run_bc_first=True):
    cfg = PPOConfig()
    device = cfg.device
    ...
    policy = FighterPolicyNet(obs_dim=obs_dim, hidden_dim=256).to(device)  # 256 instead of 128
    value_net = ValueNet(obs_dim=obs_dim, hidden_dim=256).to(device)  # 256 instead of 128

    # Optional BC warm-start
    if run_bc_first:
        print("=== Running Behavioral Cloning (BC) pretraining ===")
        train_bc(policy, env, device=device,
                 n_episodes=200, bc_epochs=20, batch_size=4096)
    else:
        print("=== Skipping BC (PPO from scratch) ===")
    ...
    while global_step < cfg.total_steps:
        ...
        while steps_this_rollout < cfg.steps_per_rollout and global_step < cfg.total_steps:
            # Policy & value
            with torch.no_grad():
                mu = policy(obs_t)
            ...
            # Episode stats - accumulate reward for current episode
        ...
        print(f"[PPO] steps={global_step} | mean_return(last10)={mean_ret:.1f} | "
        ...
        ax.set_title("Fighter PPO Training")
        plt.pause(0.01)

        # Save checkpoints (policy + value)
        torch.save(policy.state_dict(), cfg.policy_ckpt)
        torch.save(value_net.state_dict(), cfg.value_ckpt)

    plt.ioff()
    plt.show()
    print(f"[PPO] Finished training. Saved policy to {cfg.policy_ckpt}")
...
if __name__ == "__main__":
    # Set this to False if you ever want to skip BC and go pure PPO.
    RUN_BC_FIRST = True
    train_ppo(run_bc_first=RUN_BC_FIRST)
...
MISS_BOOST_THRUST = 30000.0   # instead of 60000 # Thrust force (N)
MISS_BOOST_TIME   = 3.0       # instead of 4.0   # Duration (s)

# Sustain phase: Lower thrust to maintain speed
MISS_SUSTAIN_THRUST = 5000.0  # instead of 10000 # Thrust force (N)
MISS_SUSTAIN_TIME   = 7.0     # instead of 20.0  # Duration (s)
...
N_PN            = 3.5     # was 5.0 # more aggressive PN
...
MIDCOURSE_GAIN  = 3.0     # was 4.0 # steer harder mid-course
...
class GuidanceSystem:
    ...
    def estimate_target_accel(self, target_vel, dt):
        ...
        Includes magnitude clamping to prevent spikes on mode transitions.
        Parameters
        ----------
        target_vel : np.ndarray
        ...
        # Clamp to reasonable maximum (20g) to prevent spikes from stale data
    ...
    def update(self, t, dt, missile_pos, missile_vel, 
        ...
        if has_lock:
            self.mode = 'terminal'
            # Reset acceleration estimate on mode transition to prevent spikes
            ...
            cmd = self.compute_midcourse_command(missile_pos, missile_vel,
                                                target_pos, target_vel)
            # Reset histories when transitioning from terminal to midcourse
            self.prev_los = None
            self.prev_target_vel = None
...
# ============================================================================
# RL POLICY HELPER (PLUG-IN NEURAL NET FOR THE FIGHTER)
# ============================================================================

class FighterPolicyNet(nn.Module):
    """
    Simple MLP: observation -> acceleration direction + magnitude.
    You will train this with RL offline and save weights to a .pt file.
    """
    def __init__(self, obs_dim=12, hidden_dim=128):
        ...
        self.net = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(hidden_dim, 4)  # 3 for direction, 1 for magnitude
        )
...
class FighterRLPolicy:
    """
    Thin wrapper around FighterPolicyNet that:
    - builds the model
    - loads weights (optional)
    - maps observation -> world-frame accel vector, g-limited
    """
    ...
    def __init__(self, model_path: Optional[str] = None, device: str = "mps",
        ...
        # Try loading weights if a path is given
        if model_path is not None:
            try:
                state = torch.load(model_path, map_location=device)
                self.model.load_state_dict(state)
                print(f"[FighterRLPolicy] Loaded weights from {model_path}")
            except Exception as e:
                print(f"[FighterRLPolicy] Could not load model ({e}). Using untrained net.")

    def get_action(self, obs: np.ndarray, fwd: np.ndarray,
                   max_total_accel: float) -> np.ndarray:
        """
        Map observation to an acceleration vector in *world* coordinates.

        We interpret the NN output as:
        - First 3: direction in a local maneuver plane (perpendicular to fwd)
        - Last 1: magnitude in g's (0..1 scaled to 0..max_total_accel)

        If torch or model unavailable, returns zeros.
        """
        if (not self.enabled) or (self.model is None):
            return np.zeros(3, dtype=float)

        # Ensure obs shape (obs_dim,)
        ...
        with torch.no_grad():
            ...
            out = self.model(inp)[0].cpu().numpy()  # shape (4,)

        dir_raw = out[:3]
        mag_raw = out[3]

        # Build a local maneuver basis: fwd, right, up_local
        ...
        # Project network direction into lateral plane (right / up_local)
        # dir_raw = [d_r, d_u, d_extra] but we just use first two as weights.
        d_r, d_u, _ = dir_raw
        dir_local = d_r * right + d_u * up_local
        d_norm = np.linalg.norm(dir_local)
        if d_norm < 1e-6:
            # If NN gives near-zero direction, just don't accelerate
            return np.zeros(3, dtype=float)
        dir_unit = dir_local / d_norm

        # Magnitude: squash mag_raw with tanh, then map 0..1 -> 0..max_total_accel
        mag_scale = float(np.tanh(mag_raw))  # in (-1,1)
        mag_scale = max(0.0, mag_scale)      # no negative (no "brakes" here)
        accel_mag = mag_scale * max_total_accel

        return accel_mag * dir_unit
  
# ============================================================================
# SECTION 15: TARGET AIRCRAFT CLASS (RL-ENABLED AI PILOT)
# ============================================================================

class TargetAircraft:
    """
    AI-controlled fighter that can be driven by:
      - a built-in heuristic pilot, and/or
      - an RL neural-net policy (FighterRLPolicy).

    By default, behavior = 100% heuristic, 0% RL.
    After you train a policy and load weights, you can blend:
        rl_blend = 1.0  → pure RL
        rl_blend = 0.5  → 50/50 heuristic + RL
    """
    ...
    def __init__(self, start_pos, rng,
            ...
            rl_blend: float = 0.0):
        """
        Initialize AI-controlled fighter.
        ...
        rng : np.random.Generator
            Random number generator
        rl_model_path : str or None
            Path to saved PyTorch weights for FighterPolicyNet.
            If None, RL is still wired but starts untrained.
        rl_blend : float
            Blend factor in [0,1]:
                0.0 => pure heuristic pilot
                1.0 => pure RL
        """
        self.start_pos = start_pos.astype(float).copy()
        self.pos = start_pos.astype(float).copy()
        self.vel = np.array([TARG_INITIAL_SPEED, 0.0, 0.0], dtype=float)
        self.rng = rng

        # Altitude band the fighter tries to stay inside (for heuristic pilot)
        self.alt_band = (9000.0, 13000.0)
        self.altitude_cmd = np.clip(self.pos[2], *self.alt_band)
        self.last_jink_time = 0.0

        # RL policy hook
        self.rl_blend = float(np.clip(rl_blend, 0.0, 1.0))
        self.rl_policy = FighterRLPolicy(
    ...
    # ----------------------------------------------------------------------
    # Heuristic pilot (previous AI logic)
    # ----------------------------------------------------------------------
    def _update_altitude_target(self, t: float):
        """Occasionally pick a new desired cruise altitude inside the band."""
        if t - self.last_jink_time > 5.0:
            self.altitude_cmd = self.rng.uniform(*self.alt_band)
            self.last_jink_time = t

    def _compute_heuristic_accel(self, t, missile_pos, missile_vel) -> np.ndarray:
        """
        Old AI pilot logic (no neural nets):
        - altitude-keeping
        - range-based threat logic
        - beam/break turns + random jinks
        """
        max_total_accel = TARG_MAX_G * 9.81

        if missile_pos is None or missile_vel is None:
            return np.zeros(3, dtype=float)

        # Geometry
        r = missile_pos - self.pos
        R = np.linalg.norm(r)
        if R < 1e-3:
            return np.zeros(3, dtype=float)
        ...
        # Beam / break lateral accel perpendicular to fwd & away from missile
        los_away = -los
        ...
        # Enforce g-limit including gravity
        total = a_cmd + GRAVITY
    ...
    # RL + heuristic blend
    # ----------------------------------------------------------------------
    def _build_observation(self, missile_pos, missile_vel) -> np.ndarray:
        """
        Build a 12D observation vector for RL:
    ...
    # Public update
    # ----------------------------------------------------------------------
    def update(self, t, dt, missile_pos=None, missile_vel=None):
        """
        Advance fighter state by one time step using:
        - heuristic pilot
        - optional RL policy (blended via rl_blend).
        ...
        dt : float
            Time step (s)
        missile_pos : np.ndarray or None
            Missile position (m)
        missile_vel : np.ndarray or None
            Missile velocity (m/s)
        ...
        # Heuristic command
        a_heur = self._compute_heuristic_accel(t, missile_pos, missile_vel)

        # RL command (if enabled & we have missile info)
        if missile_pos is not None and missile_vel is not None and self.rl_blend > 0.0:
            obs = self._build_observation(missile_pos, missile_vel)
            speed = np.linalg.norm(self.vel)
            if speed < 1e-3:
                fwd = np.array([1.0, 0.0, 0.0], dtype=float)
            else:
            ...
            a_rl = self.rl_policy.get_action(obs, fwd, max_total_accel)
        else:
            a_rl = np.zeros(3, dtype=float)

        # Blend: a_cmd = (1 - alpha)*heuristic + alpha*RL
        alpha = self.rl_blend
        a_ai = (1.0 - alpha) * a_heur + alpha * a_rl

        # Add gravity
        a_total = a_ai + GRAVITY
        ...
        speed = np.linalg.norm(self.vel)
        if speed > 1e-3:
            desired_speed = TARG_INITIAL_SPEED
            self.vel *= desired_speed / speed
...
def simulate_engagement(aircraft_start, missile_start, rng=None,
    ...
    target = TargetAircraft(
        aircraft_start,
        rng,
        rl_model_path="fighter_policy_continued.pt",
        rl_blend=0.9 #1.0 for full RL  # Pure RL policy
    )
    ...
    for i, t in enumerate(times):
        # Update target with AI pilot that sees the missile
        target.update(t, DT, missile.pos, missile.vel)
...
def create_animation(times, target_states, missile_states, missile_speeds,
    ...
    target_path_line, = ax.plot(
        target_states[:, 0], target_states[:, 1], target_states[:, 2],
        'b--', linewidth=1, alpha=0.3
    )
    
    # Start markers
    aircraft_start_scatter = ax.scatter(
        *aircraft_start, c='green', s=100, marker='s', zorder=5
    )
    missile_start_scatter = ax.scatter(
        *missile_start, c='orange', s=100, marker='^', zorder=5
    )
    ...
    ax.legend(
        [target_path_line, aircraft_start_scatter, missile_start_scatter],
        ['Target Path', 'Aircraft Start', 'Missile Start'],
        loc='upper left',
        fontsize=8,
        framealpha=0.8,
    )
    ...
    def update(frame):
        ...
        mode_txt.set_text(f'Mode: {"TRM" if lock >= 0 else "MID"}')
...
if __name__ == "__main__":
    ...
    for k in range(n_trials):
        ...
        if result['intercept_real']:
            ...
            n_decoy += 1
        elif result.get('intercept_ground'):
            n_miss += 1  # Ground impact counts as miss
        else:
            n_miss += 1
